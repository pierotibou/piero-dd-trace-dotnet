...
internal class JsonTelemetryTransport : ITelemetryTransport
{
    ...
    public JsonTelemetryTransport(IApiRequestFactory requestFactory, Uri baseEndpoint)
    {
    ...
    public async Task<TelemetryPushResult> PushTelemetry(TelemetryData data)
    {
        try
        {
            ...
            {
                Log.Debug("Error sending telemetry: 404. Disabling further telemetry, as endpoint not found");
                return TelemetryPushResult.FatalError;
            }
            else
            {
                Log.Debug<int>("Error sending telemetry {StatusCode}", response.StatusCode);
