...
module SpanModel =
    ...
    let isSpan : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isRootSpan : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        tagIsPresent "language"
        &&& tagIsPresent "runtime-id"
        &&& metricMatches "_dd.top_level" 1.0

    let isAspNetCore : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isAspNetCoreMvc : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isElasticsearch : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let ``isGraphQL Server`` : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isHttpMessageHandler : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        matches name "http.request"
        &&& matches ``type`` "http"
    ...
    let isMongoDB : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isPostgreSQL : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isRabbitMQ : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let ``isService Fabric`` : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        matches name "amqp.command"
        &&& matches ``type`` "redis"
    ...
    let ``isService Remoting (client)`` : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let ``isService Remoting (server)`` : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isServiceStackRedis : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
    ...
    let isStackExchangeRedis : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        matches name "redis.command"
        &&& matches ``type`` "redis"
        &&& tagIsPresent "redis.raw_command"
        &&& tagIsPresent "out.host"
    ...
    let ``isWcf (server)`` : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        matches name "wcf.request"
        &&& matches ``type`` "web"
        &&& tagIsPresent "http.url"
        &&& tagMatches "span.kind" "server"

    let isWebRequest : MockTracerAgent.Span -> Result<MockTracerAgent.Span, string> =
        matches name "http.request"
        &&& matches ``type`` "http"
        &&& tagIsPresent "component"
        &&& tagIsPresent "http-client-handler-type"
